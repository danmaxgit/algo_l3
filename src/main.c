...
int main(){
    ...
    cpus[2].type = T3;
    Task t;
    while (gen->index > 0 || stack->index > 0 || cpus[0].task.duration > 0 || \
    cpus[1].task.duration > 0 || cpus[2].task.duration > 0) {
        ...
        if (stack->index > 0) {
            for (int i = 0; i < 3; i++) {
                if(cpus[i].type == decode(stack->data[stack->index - 1]).type && cpus[i].task.duration <= 0){
